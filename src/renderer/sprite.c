...
/*   Updated: 2022/06/12 22:43:03 by tkim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_spritedata	spritedata__init(
	const t_sprites sprites, t_camera *camera, int i)
{
	t_spritedata	s;
			...
			s.transform.y < this->zbuffer[stripe])
		{
			...
			while (++y < s.y_range.end)
			{
				...
				* TEX__WIDTH / s.size.width) / 256);
				...
				int color = this->world.texture[TEX__SPRITE0 + is_other_frame(frame)][TEX__WIDTH * texy + texx];
				if ((color & 0xFFFFFF) != 0)

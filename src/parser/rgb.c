...
void	rgb__print(t_rgb rgb)
{
	...
	printf(BWHT "[" HRED "%03d " HGRN "%03d " HBLU "%03d" BWHT "]" END, r, g, b);
}
...
t_rgb	rgb__from_string(t_const_string str, t_const_string delim)
{
	...
	if (arr_len != 3)
		std__panic__index("parser__parse_rgb", arr_len, 3);
	i = -1;
	while (++i < 3)
	{
		parse_res = str__to_int(arr[i]);
		if (parse_res.err)
			std__panic__value("parser__parse_rgb", arr[i]);
		raw_rgb[i] = parse_res.num;
		if (not math__in_range(raw_rgb[i], (t_irange){0, 255}))
			std__panic__index("parser__parse_rgb", raw_rgb[i], 255);
	}
